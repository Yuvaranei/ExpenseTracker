    ...
    render()
    {
        ...
        else
        {
            ...
            this.state.expenses.map((item, index)=>
            {
                ...
                tableData.push(<tr key={index}><td><input name="expenseRadio" type="radio" id={index} onChange={that.handleRadio}/>{item.category}</td><td>{item.amount}</td><td>{displayDate}</td></tr>);
            })
        }
